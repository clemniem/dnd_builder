...
object SRD_registry {
  ...
  private def feat(id: String, name: String, description: String, uses: Option[Uses], informative: Boolean): (String, Feature) =
    id -> Feature(id, name, description, uses, Nil, informative)

  private def featWith(id: String, name: String, description: String, uses: Option[Uses], grants: List[FeatureGrant], informative: Boolean): (String, Feature) =
  ...
  private val all: Map[String, Feature] = List(
    // --- Level 1 (classes) ---
    feat("rage", "Rage", "2 uses. +2 damage, resistance to B/P/S, Advantage on STR checks. Bonus Action to activate.", Some(Uses.Static(2)), false),
    ...
    feat("bardic-inspiration", "Bardic Inspiration", "d6 die. Bonus Action to grant to ally within 60ft.", Some(Uses.AbilityMod(Ability.Charisma)), false),
    ...
    feat("second-wind", "Second Wind", "2 uses. Bonus Action to heal 1d10 + Fighter level HP.", Some(Uses.Static(2)), false),
    ...
    feat("favored-enemy", "Favored Enemy", "Hunter's Mark always prepared. 2 free casts per Long Rest.", Some(Uses.Static(2)), false),
    ...
    feat("innate-sorcery", "Innate Sorcery", "2 uses. Bonus Action for 1 min: Advantage on spell attacks, spell save DC +1.", Some(Uses.Static(2)), false),
    ...
    feat("arcane-recovery", "Arcane Recovery", "Once per day on Short Rest, recover spell slots totaling up to half Wizard level (round up).", Some(Uses.Static(1)), false),
    ...
    feat("channel-divinity-cleric", "Channel Divinity", "2 uses per Short/Long Rest. Turn Undead: undead within 30ft must save or be turned for 1 min.", Some(Uses.Static(2)), false),
    feat("wild-shape", "Wild Shape", "Bonus Action to magically assume a beast form.", Some(Uses.ProfBonus()), false),
    feat("wild-companion", "Wild Companion", "Expend a Wild Shape use to cast Find Familiar without material components.", None, false),
    feat("action-surge", "Action Surge", "1 use per Short/Long Rest. On your turn, take one additional Action.", Some(Uses.Static(1)), false),
    ...
    feat("monks-focus", "Monk's Focus", "2 Focus Points per Short/Long Rest. Spend points for Flurry of Blows (1), Patient Defense (1), or Step of the Wind (1).", Some(Uses.Static(2)), false),
    ...
    feat("font-of-magic", "Font of Magic", "2 Sorcery Points per Long Rest. Convert spell slots to points or points to slots.", Some(Uses.Static(2)), false),
    feat("metamagic", "Metamagic", "Choose 2 Metamagic options. Spend Sorcery Points to twist spells.", None, false),
    feat("magical-cunning", "Magical Cunning", "If all Pact Magic slots are expended, perform a 1-minute ritual to recover half (round up). Once per Long Rest.", Some(Uses.Static(1)), false),
