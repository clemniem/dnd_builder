...
object NotesScreen extends Screen {
  ...
  private def createForm(model: Model): Html[Msg] =
    div(`class` := s"${NesCss.containerRounded} gallery-card", style := "flex-direction: column; align-items: stretch;")(
      ...
      button(`class` := NesCss.btnPrimary, onClick(NotesMsg.Create))(text("+ Add Note"))
    )
      ...
      (
        if (pendingDeleteId.contains(note.id))
          ...
          GalleryLayout.galleryActionsRow(
            button(`class` := NesCss.btnError, onClick(NotesMsg.AskDelete(note.id)))(text("Delete"))
          )
      )
    )
}
